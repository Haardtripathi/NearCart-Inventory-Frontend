...
function VariantEditor({
  ...
  const generatedSkuPreview = generateVariantSku(productName, variantName, index)
    ...
    <div className="space-y-4 rounded-md border border-slate-200 bg-slate-50/80 p-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Controller control={control} name={`variants.${index}.sku`} render={({ field, fieldState }) => (
          <FormField
            label={t('sku')}
            description={sku?.trim() ? 'Edit this SKU if you need a custom code.' : `Will save as ${generatedSkuPreview} if left blank.`}
